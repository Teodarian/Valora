...
- `app.run(debug=True, host="0.0.0.0", port=5000)` — må ha host="0.0.0.0" for nettverkstilgang
...
- [ ] DuckDNS — gratis DDNS (solvr.duckdns.org)
...
## 6. Endringer denne sesjonen (2026-06-10, Kaushik — sesjon 2)

- Begge Flask-appene deployet på Linux VM (Ubuntu 26.04, VMware Fusion)
- Git repo klonet til `/srv/Valora` på VM
- botbyra-portfolio submodule-problem løst — konvertert til vanlig mappe i GitHub
- Python venv satt opp for begge apper
- Systemd-tjenester opprettet: `bottbyra.service` og `vendera.service`
- Nginx installert og konfigurert for bottbyra.local og vendera.local
- VM byttet fra NAT til Bridged (Wi-Fi) i VMware Fusion
- Statisk IP satt til 10.0.0.200 via netplan
- Vendera app.py oppdatert: `host="0.0.0.0"` lagt til i app.run()
- Begge apper tilgjengelige på lokalt nettverk fra alle enheter via IP
- Mac hosts file oppdatert med .local-hostnames

---

## 7. Nærmeste neste steg (prioritert)

1. **[Infra — Medium]** Sett opp DuckDNS for DDNS
2. **[Infra — Medium]** GitHub Action for auto-deploy ved git push
3. **[Infra — Low]** Router DNS for .local på hele nettverket (mobil-tilgang via hostname)
4. **[BottByrå — Medium]** Rydd opp unødvendige elementer og feil ordbruk på nettsiden
5. **[BottByrå — Medium]** Sett opp Cal.com booking-lenker
6. **[BottByrå — Medium]** Test alle 3 botts på mobil (10 spørsmål hver)
7. **[Vendera — Critical]** Koble frontend auth til Flask-backend (/api/register, /api/login, /api/logout, /api/me)
8. **[Vendera — Medium]** Fyll ut requirements.txt med flask og flask-sqlalchemy
